...
install:
	...
	cp -f ssg-build $(DESTDIR)$(PREFIX)/bin
	chmod 755 $(DESTDIR)$(PREFIX)/bin/ssg-build
	mkdir -p $(DESTDIR)$(MANPREFIX)/man1
	cp -f man/injectindex.1 $(DESTDIR)$(MANPREFIX)/man1/
	...
	chmod 644 $(DESTDIR)$(MANPREFIX)/man1/injectindex.1
...
uninstall:
	rm -f $(DESTDIR)$(PREFIX)/bin/ssg-build
	rm -f $(DESTDIR)$(MANPREFIX)/man1/st.1
	rm -f $(DESTDIR)$(MANPREFIX)/man1/injectindex.1 
...
install_split:
	...
	cp -f injectindex $(DESTDIR)$(PREFIX)/bin
	...
	chmod 755 $(DESTDIR)$(PREFIX)/bin/injectindex
...
uninstall_split:
	...
	rm -f $(DESTDIR)$(PREFIX)/bin/injectindex
	rm -f $(DESTDIR)$(PREFIX)/bin/markdown2html
	rm -f $(DESTDIR)$(PREFIX)/bin/wraphtml

.PHONY: install uninstall install_split uninstall_split
